...
impl IdentityManager {
    pub fn new(env: &dyn Environment) -> DfxResult<Self> {
        ...
        let configuration = if identity_json_path.exists() {
            ...
            initialize(&identity_json_path, &identity_root_path)
        }?;
...
fn initialize(identity_json_path: &Path, identity_root_path: &Path) -> DfxResult<Configuration> {
    println!(r#"Creating the "default" identity."#);
    ...
    if !identity_pem_path.exists() {
        ...
        if creds_pem_path.exists() {
            ...
            println!("  - generating new key at {}", identity_pem_path.display());
    ...
    println!(r#"Created the "default" identity."#);
